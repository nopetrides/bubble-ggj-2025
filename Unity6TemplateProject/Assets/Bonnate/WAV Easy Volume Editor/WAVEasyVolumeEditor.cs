...
namespace Bonnate
{
	public class WAVEasyVolumeEditor : EditorWindow
	{
		...
		private void OnEnable()
		{
			...
			var imagePath = $"{mScriptFoliderPath}/Images/ListenButtonImage.png";
		...
		private void OnGUI()
		{
			...
			if (selectedAudioClip != null)
			{
				...
				var assetPath = AssetDatabase.GetAssetPath(selectedAudioClip);
			...
			selectedAudioClip =
				EditorGUILayout.ObjectField("", selectedAudioClip, typeof(AudioClip), false, GUILayout.Width(180)) as
					AudioClip;
			...
			if (GUILayout.Button(mListenBtnTexture, GUILayout.Width(20), GUILayout.Height(20))) ListenToAudio();
			...
			if (GUILayout.Button("Process and Replace"))
			{
				...
				{
					var (samples, channels, sampleRate) = ProcessAndExport();

					// Get the AudioClip's path
					var audioClipPath = AssetDatabase.GetAssetPath(mAudioClip);

					// Create the backup file path and make a backup
					if (File.Exists(audioClipPath))
					{
						var backupPath = Path.Combine(mScriptFoliderPath, "_Backups", Path.GetFileName(audioClipPath));
						File.Copy(audioClipPath,
							$"{backupPath.Replace(".wav", "")}_bak{DateTime.Now.ToString("_yyMMdd_HHMMss")}.wav", true);
					}
			...
			if (GUILayout.Button("Process and Export"))
			{
				...
				else
				{
					...
					// Get the file path
					var outputPath = EditorUtility.SaveFilePanel("Save Processed WAV", "", "processed.wav", "wav");
			...
			if (GUILayout.Button("Github", GetHyperlinkLabelStyle())) OpenURL("https://github.com/bonnate");

			if (GUILayout.Button("Blog", GetHyperlinkLabelStyle())) OpenURL("https://bonnate.tistory.com/");
		...
		{
			if (mAudioClip == null) return 0f;

			var samples = new float[mAudioClip.samples * mAudioClip.channels];
			...
			for (var i = 0; i < samples.Length; i++)
			{
				...
				var db = 20.0f * Mathf.Log10(sample);
				if (db > mMaxDb) mMaxDb = db;
			}
		...
		{
			var sampleRate = mAudioClip.frequency;
			var channels = mAudioClip.channels; // Get the channel count from the original WAV file

			var samples = new float[mAudioClip.samples * channels];
			mAudioClip.GetData(samples, 0);

			// Calculate the multiplier to adjust the decibel value
			var dbDifference = -mTargetDb - mMaxDb;
			var multiplier = Mathf.Pow(10.0f, dbDifference / 20.0f);

			// Adjust the decibel value by multiplying the multiplier to all samples
			for (var i = 0; i < samples.Length; i++) samples[i] *= multiplier;
		...
		private void SaveWav(string path, float[] samples, int channels, int sampleRate)
		{
			using (var writer = new BinaryWriter(File.Open(path, FileMode.Create)))
			{
				...
				// Write WAV data
				foreach (var sample in samples) writer.Write((short)(sample * 32767.0f));
			}
		}
		...
		private void ListenToAudio()
		{
			...
			var audioSource = EditorUtility
				.CreateGameObjectWithHideFlags("AudioSource", HideFlags.HideAndDontSave, typeof(AudioSource))
			...
			if (-mTargetDb < mMaxDb)
			{
				...
				var maxVolume = Mathf.Pow(10.0f, -mTargetDb / 20.0f);
			...
			else
			{
				...
				var tempAudioClip = AudioClip.Create("ProcessedAudioClip", samples.Length / channels, channels,
					sampleRate, false);
